    ...
    <x-slot name="slot">
        <center>
            <div class="card shadow mb-4 col-xs-12 col-md-8 border-bottom-primary">
                ...
                </div><a  type="button" class="btn btn-primary bg-gradient-primary btn-block" href="{{route('view.abonner')}}"> <i class="fas fa-flip-horizontal fa-fw fa-share"></i> Retour </a>
                @include('layouts.flash-message')
                <div class="card-body">
{{--                    {{ dd($formule, $abonnement,$datas,$decodeur) }}--}}
                    <form role="form" method="post" action="{{route('abonnement.upgrade.save')}}">
                        @csrf
                        <input type="hidden" name="id_client" value="{{ $datas->id_client }}">
                        <input type="hidden" name="id_abonnement" value="{{ $abonnement[0]->id_abonnement }}">
                        <input type="hidden" name="id_decodeur" value="{{ $abonnement[0]->id_decodeur }}">
                        <input type="hidden" name="id_formule" value="{{ $abonnement[0]->id_formule }}">
                        ...
                        <input type="hidden" name="date_reabonnement" value="{{ $decodeur[0]->date_abonnement }}">
                        ...
                        <div class="form-group">
                            Date reabonnement<br><input class="form-control" value="{{ $abonnement[0]->date_echeance }}" name="date_reabonnement" type="date" readonly required>
                        </div>
                        <hr>
